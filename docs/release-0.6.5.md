# ALLIN1 SDK 0.6.5 — release guide

0.6.5 is a maintainer-approved **unsigned manual-download release**. SDK
...
The maintainer approved publication on September 7, 2026. No publisher signing
...
| Windows lifecycle | Final-byte native lifecycle is not claimed; consult the packaged portable rehearsal report for its narrower tested scope |
| Desktop observations | Earlier native observations are historical; final-build checks are identified separately in candidate reports |
...
0.6.5. Any skipped checks remain untested and block full qualification.
