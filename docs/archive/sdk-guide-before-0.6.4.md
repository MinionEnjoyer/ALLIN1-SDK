# Historical sdk reference — before the 0.6.4 documentation pass

> Historical material, retained without treating its instructions or results as current release guidance.
> The current entry point is the repository README and 0.6.4 release guide.
...
- **Quick Import** — use a separate guided workspace when the goal is packaging,
  ...
  of unrelated files. It links primary and high-detail YFT fragments, same-stem
  package-owned YBN collision dictionaries, YTD textures, handling, variations,
  ...
  directly into the embedded Asset Viewer and texture-authoring workflow. A separate
  collision toggle composites exact YBN triangles and clearly-labelled diagnostic box
  hulls in the same model-local camera; capsules, cylinders, spheres, and unsupported
  primitives remain count-only evidence rather than guessed geometry. UV mode also
  produces a texture-grouped flattened UV0 atlas from the same Python-decoded topology:
  same-tile triangles form stable shared-edge islands over bounded linked-YTD backgrounds,
  while cross-tile seams, degenerate coordinates, and missing UV0 remain explicit counts
  instead of invented polygons. **Build
...
The next Model & Materials depth pass will promote the proven native viewport,
texture-grouped UV0 atlas, exact Vector4 shader constants, and same-camera YBN
evidence into the standalone Tauri workspace. The first authoring actions will
follow the current rule: preserve unknown XML, modify only proven existing shader,
sampler, or ShaderIndex fields, retain revision and hash history, and compile/reparse
outside GTA V before a result can be published.
...
The optional assistant can be configured directly in the Tauri SDK's
**Qwen Assistant → Standalone setup**. Use a compatible API or your own trusted
llama.cpp server and GGUF model. Saving settings does not download or run anything.
Settings live in `%LOCALAPPDATA%\ALLIN1-SDK\Assistant\config.json`; API keys stay
in environment variables, not in this file. Existing Launcher assistant settings
are read as a compatibility fallback only when SDK settings are absent.
The Launcher's **SDK Manager → Optional assistant** remains an optional way to
install managed model packs. Qwen is not required for any other SDK feature.
...
The **Tauri migration build** produces `ALLIN1 SDK_<version>_x64-setup.exe` and
a matching `.sha256` file under `desktop/src-tauri/target/release/bundle/nsis/`.
Verify the checksum, run the installer, and open **ALLIN1 SDK** from the Start
menu. No Launcher, ALLIN1 game client, Python, or separately installed .NET runtime
is required. The installer includes the Python sidecar, self-contained RPF helper,
schemas, templates, documentation, and runtime authoring sources. WebView2 is
required by the shell and is handled by the installer if missing.
This is a separate migration-build installer, not the older Launcher ZIP format;
do not rename its files to satisfy Launcher v0.5.0's legacy archive checks.

For the existing **Tkinter public release**, the direct portable option remains:
...
The Tauri v2 desktop now gives **Models & Materials** its own native workspace
for loose YDR/YDD/YFT assets, with aligned shader, geometry, and rendered-evidence
panes backed by the same Python decoder and React viewport as the Vehicle
Workbench. After inspection, it can create or reopen an isolated editable copy,
review shader-name, texture-binding, and geometry-assignment diffs, commit them
against an exact revision, and undo through verified history without touching
the original model or GTA V. Existing numeric Vector and Array shader parameters
are preserved as exact Vector4 rows and can be edited component by component
through the same revision- and digest-bound review flow; the SDK never invents
parameter nodes or changes their source shape. A reviewed build can now compile to a new native
asset outside both the workspace and game, decode it again, publish a SHA-256
validation receipt, and compare the source and rebuilt renders side by side in
React. A second aligned **Texture dictionaries** area moves the core YTD editor
into React: it creates or reopens copied native workspaces, previews exact DDS
dependencies, reviews raster/DDS add or replace operations and removals against
the current workspace digest, supports revisioned undo, and builds only new YTD
outputs whose reparse receipt proves both the output hash and semantic XML match.
The docked console also exposes Qwen, including SDK-owned standalone setup,
through typed status and structured advisory jobs. Assistant
answers cannot execute commands or write files, and stopping a request tears
down its isolated local runtime process tree.
...
The Tauri v2 Weapon Workbench also supports reviewed, undoable first-person
camera/scope offsets, rotations, FOV and the complete weapon behavior-flags list.
Its calibration helper proposes a translation from measured reference/custom
sight anchors and optionally estimates FOV from magnification; it does not infer
alignment from model names or replace in-game verification. See
[Weapon scope authoring](docs/weapon-scope-authoring.md).
