"""Display-independent SDK help catalog for React and service consumers."""
...
The read-only assistant group is also available through execute requests: pass command assistant and args beginning with context or prompt. Vehicle Quick Import is exposed as the read-only inspect-vehicle-quick-import command and the authoring-only prepare-vehicle-quick-import command; preparation may create a package in the shared library but never writes GTA V. The host supplies focused repository, workspace, manifest, game-path, dirty-state, and live command evidence under a permanent ALLIN1 policy. A prompt response cannot approve or invoke another command. This API remains the stable foundation for later reviewed AI-assisted package inspection and approved package lifecycle actions. The ALLIN1 Launcher remains the user-facing owner of assistant installation, interactive package installation, and game launch.""",
    ...
    HelpTopic(
        ...
        """The product header keeps the SDK identity, version, support link, and active-workspace marker in one consistent place. It exposes one compact contextual return link only when there is somewhere useful to go back to.
...
Workspace tools > Open YTD texture workspace switches this same window into a searchable texture editor. It previews DDS images, imports DDS or common raster formats, updates width/height/mip/format metadata, supports add/replace/remove, and retains local undo history. Build + validate YTD reparses the completed dictionary before it is accepted. Raster imports become uncompressed DDS; provide a prepared DDS when compression and mip control must be retained. Compiled DLL, ASI, and script payloads are never executed.""",
    ...
    HelpTopic(
        ...
        """Open a loose YDR, YDD, or YFT model, a direct DLC RPF, or an extracted/package archive and choose one model from its inventory. The left navigator separates renderable models from related YTD textures, YBN collision, and YTYP archetype context. Related assets are evidence, not guessed dependencies. Direct RPFs are read-only here; export an editable workspace before changing native assets.
...
Vehicle Quick Import scans a direct dlc.rpf, package archive, or extracted folder, shows only editions proven by its vehicle metadata and recursively inspected RPF branch, and lets you review each GBAY listing. A direct RPF must contain one unambiguous registered DLC pack name. Technical model labels are made readable when that can be done safely, known manufacturers may be recognized from an unambiguous package filename, and vehicle size is shown as Standard, Large, or Oversize while retaining the numeric package contract. A zero price requires explicit confirmation. Traffic is off by default and becomes eligible only when explicitly selected for a road vehicle. Aircraft and boats retain their specialized storage and cannot opt into normal road traffic.
...
Weapon and Ped Quick Import remain clearly marked as upcoming guided pipelines. Their buttons route to the matching advanced Content Workbench tab rather than producing partial or misleading packages. Console and Agent API users can inspect and prepare vehicle quick imports, open a prepared package in Launcher, or author a Legacy OIV through explicit typed commands.""",
    ...
    HelpTopic(
        ...
        "Review vehicle, weapon, ped, and map projects without reopening the package.",
        """Open a direct DLC RPF, loose add-on folder, or supported package archive once. The Workbench shows indexed-file and content counts on the Vehicles, Weapons, Peds, and Maps tabs and keeps every specialist view tied to the same evidence. Direct RPFs remain sealed and read-only; authoring buttons explain that an editable workspace must be extracted first.

Vehicles provides the existing model viewport and guarded vehicle-authoring tools. Weapons links definitions to ammo, animations, shop registration, attachment points, component models, and streamed assets, then enables supported fields only inside a verified copied authoring workspace. Peds links peds.meta definitions to drawable dictionaries, texture dictionaries, props, movement clips, and expression data, with guarded editing inside its own copied workspace. Maps inventories YMAP/YTYP/collision/navigation assets and binds them to a declarative runtime project of levels, entrances/exits, garages, and spawn/store slots.

The Package Linker and specialist content workbenches use the same compact workspace layout, including slim green divider arrows where a dense catalog or inspector can be folded away. Expanding a side restores its previous width, selection, and edit state, while the active project remains in one main SDK window.

Use Open selected asset to route a project member into the SDK Asset Viewer without creating another application window. Missing links remain visible as review findings; the Workbench does not invent metadata or execute package code. Console and Agent API command inspect-workbench returns the same records, component links, and findings as structured JSON. open-workbench accepts --category auto, vehicles, weapons, peds, or maps. open-axle-configurator accepts a vehicle authoring workspace and optional --model, then uses the normal SDK desktop to select Vehicles > Author > Axles directly. The older open-vehicle-workbench command remains a compatibility alias.""",
        (
    ...
    HelpTopic(
        "map-workbench", "Authoring", "Map Workbench",
        "Package streamed maps and define levels, entrances, exits, and garages.",
        """Open a direct DLC RPF, extracted DLC folder, or package archive in Content Workbench > Maps. The left pane inventories map-native placements, archetypes, collision, navigation, drawables, and textures. The middle pane shows the declared levels, portals, garages, and garage slots. The right pane keeps the selected record's JSON and validation result visible.

Create map project writes a valid starter descriptor to a new path. Project identity edits the package name, id, version, and declared editions. Streaming and loads edits the DLC pack, content group/IPLs, activation and release radii, and resident behavior without leaving the SDK. Add levels, entrances/exits, garages, and slots from the inline Add menu; select any record to edit its JSON, then Apply JSON. Every change reparses the complete contract before it becomes the in-memory draft, so broken references or invalid coordinates never replace the last valid state. Save descriptor performs an atomic local-file replacement. Garage changes are explicitly story-save-only.

A portal connects the reserved world level to a named map level, or connects two named levels, with pedestrian, vehicle, or combined traversal. Garages reference one connecting portal and declare capacity, allowed vehicle families, and exact spawn/store slots. Streaming declares one standalone DLC pack plus a content group or IPL names and bounded activation/release radii. Detect installed IPLs recursively reads the selected GTA installation's matching DLC root RPFs, including Enhanced split archives such as dlc1.rpf, lists every YMAP name with root/archive/entry provenance, and verifies the descriptor's requested IPLs without changing the descriptor or game files. Ambiguous matches remain blocked for review.

Build package selects one declared edition and publishes into a new output folder. It accepts one direct dlc.rpf, one packaged dlc.rpf, or a dlc.rpf.source directory when the matching GTA installation is available. The result contains a schema-2 managed package, exact hashes, DLC registration, the runtime maps.json destination, content capability world.maps, and a retained build report. It never installs the package or writes GTA V. Console and Agent API commands validate-map-project, inspect-map-project, detect-map-placements, and build-map-package expose the same contract, read-only discovery, and guarded builder.""",
        (
            "map", "ymap", "ytyp", "garage", "portal", "entrance", "exit",
            "level", "streaming", "world.maps", "detect-map-placements",
            "build-map-package",
        ),
    ),
    HelpTopic(
...
In the Tauri desktop, choose Content Workbench > Peds. Open ped folder or Open archive inspects without writing; Create editable copy reviews a new destination before copying. Open editable copy resumes an existing workspace. The Ped section selector exposes Definition, Author fields, Identity + assets, New from template, Preview and Asset family. Asset family opens the exact selected file inline. Preview requires an explicit decoder edition when the package does not identify one and keeps same-named asset paths separate. Each mutation has a review and separate confirmation; field, identity and clone drafts block navigation until applied or discarded. Ctrl+F focuses Filter peds and Escape clears a clean filter. Duplicate ped definitions are retained for inspection, but ambiguous identities cannot be edited. Present files and declared relationships are not runtime compatibility, YMT capacity or in-game acceptance evidence.
...
Console and Agent API commands create-ped-authoring, inspect-ped-authoring, plan-ped-clone, clone-ped-bundle, set-ped-fields, migrate-ped-identity, and undo-ped-edit expose the same guarded implementation. plan-ped-clone is read-only; clone-ped-bundle requires the exact --expected-revision, exact --plan-sha256, and --acknowledge-edit. All authoring operations affect only the copied workspace and never write to the original package or GTA V installation.""",
    ...
    HelpTopic(
        ...
        """Open a direct DLC RPF, loose DLC folder, or supported package archive. The workbench resolves every vehicles.meta record against its primary and high-detail YFT fragments, YTD texture dictionary, handling record, variation record, tuning kits, labels, and DLC registration evidence. A direct RPF is an inspection source; extract an editable authoring workspace before changing it.
...
Author > Hitches configures front/rear connection slots, compatible trailer models, and guarded coupling distances. Native towing uses existing attach_female/attach_male bones with no offset override. Experimental physical joints expose bone-local position, coupler offset, rotation and break force with sliders and manual input. The preview is an offset schematic, not decoded collision geometry. Review and confirm before saving; workspace undo/redo is supported. Vehicle package export includes the shared profile in its catalog. The vehicle_hitches authoring module exposes the same inspect/review/apply path to CLI, API and agents. No YFT bones or game files are changed, and live physics acceptance requires separate in-game testing.
...
Author > Axles detects two through five canonical physical axle pairs from the decoded vehicle skeleton. The table keeps bone roles, explicit runtime wheel slots, steering, drive, service/handbrake intent, and GTA's front versus shared middle/rear visual families separate. Quick presets apply immediately to the draft, while the data-driven behavior library changes the draft only after Use behavior; nothing is saved before Apply + validate. Physical order… is an advanced, explicit override for models that deliberately place GTA's canonical wheel families in a different front-to-rear order for visual instancing. It never renames bones or meshes, records the exact reviewed skeleton digest, requires the selective runtime, and clears conflicting stock rear/all-wheel steering flags. The compact Axle support bias control is experimental: when enabled it requires a 0.75–1.25 relative weight for every physical axle, saves schema 3 authoring data, and cannot be exported until the selected target explicitly exposes a validated support accessor. Filters cover axle count, nominal layout, vehicle category, steering, drive, lift behavior, target compatibility, and experimental status. Behavior prefabs never move or rename bones. Visual tyre packages are independent and dual tyres remain ordinary bone-bound geometry rather than extra physics wheels. Lift behavior is design-only, trailer steering remains experimental, and configurations beyond GTA's five recognized canonical pairs are reported as cosmetic/custom-physics work.
...
Config > Build Story controller package opens one inline builder for the generic native controller. Tool selection supports Auto discovery or strict Manual paths, with Browse controls for cmake.exe, ctest.exe, a Visual Studio/Build Tools installation, or an exact Hostx64/x64 cl.exe; these workstation-only choices are saved under the current user's SDK data and never enter a vehicle package. Auto-detect rechecks the live filesystem and machine/user PATH, while Reset clears every override. Opening the panel, or choosing Recheck, runs the complete isolated toolchain preflight and shows each selected path, version, discovery source, compatibility result, Visual Studio and MSVC component, Windows SDK, x64 compiler result, and copyable repair guidance. Build remains locked until every check, component fingerprint, and the isolated C++17/CTest probe passes. Choose Legacy, Enhanced, or both; Browse converts the selected configuration directory and log file beneath a declared GTA installation into portable forward-slash paths and rejects locations outside those roots; then select a new staging folder outside GTA. The SDK compiles the shared source with the exact preflight-selected CMake, CTest, Visual Studio instance, compiler/toolset, architecture, and Windows SDK, runs its native tests, verifies each x64 PE and edition marker, converts the current reviewed Workbench draft into an edition-specific native sidecar, validates that sidecar with the same C++ parser used by the controller, and emits checksummed archives and a build manifest. The controller is global and can load many per-vehicle JSON files; it is not rebuilt for every vehicle. A local successful build remains marked unsupported and not game-tested until it completes the separate in-game acceptance process.
...
Console and Agent API commands create-vehicle-authoring, inspect-vehicle-authoring, set-vehicle-fields, set-vehicle-appearance, set-vehicle-tuning-kit, inspect-vehicle-tuning, add-vehicle-tuning-entry, set-vehicle-tuning-entry, remove-vehicle-tuning-entry, move-vehicle-tuning-entry, set-vehicle-light-profile, migrate-vehicle-identity, undo-vehicle-edit, redo-vehicle-edit, inspect-vehicle-axles, preview-axle-steering, set-vehicle-axles, list-axle-prefabs, preview-axle-prefab, preview-axle-tyres, inspect-story-axle-toolchain, build-story-axle-runtime, plan-axle-runtime-bundle, build-axle-runtime-bundle, plan-axle-oiv, build-axle-oiv, inspect-vehicle-project, export-vehicle-project, build-vehicle-package, plan-managed-vehicle-package, export-managed-vehicle-package, and publish-managed-vehicle-package expose the same guarded implementation. Automatic steering uses canonical wheel-bone positions to propose a neutral pivot and signed per-axle gains; visual wheel meshes and dual tyres never decide steering direction. Missing or opaque data stays visible as a finding rather than being guessed.""",
        (
            ...
            "axle", "multi-axle", "steering", "drive", "suspension",
            "support bias", "prefab", "tyres",
