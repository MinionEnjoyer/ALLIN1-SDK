...
OIV preview recognizes the official XML add/replace/remove grammar as a compile workflow while keeping text, PSO, unbounded archive-creation, ambiguous XPath, and unknown operations blocked. Select the matching outer RPF to compile supported XML recipes into reparsed, canonically verified payloads and a hash-bound inert plan; the source archive is not written. Bounded createIfNotExist recipes replay declared adds, XML edits, and cleanup deletes in order before becoming exactly verified managed packages, while exact adds/replacements/deletes inside existing nested RPF trees can be exported as payload-backed batch manifests.""",
...
File > Build/update global RPF catalog recursively discovers up to 512 loose archives and atomically writes a SQLite search index outside the game installation. Unchanged size/mtime records reuse their prior recursive index; CLI --refresh forces every archive through indexing and SHA-256 again. Unreadable archives are recorded without hiding valid results. File > Search global RPF catalog searches outer names, nested archive paths, and entry paths; double-click a result to open its outer archive and select the exact nested entry.
...
OIV preview recognizes bounded createIfNotExist recipes instead of executing them. The CLI's --created-rpf-package workflow extracts only declared content members into retained loose authoring sources, replays supported XML edits and cleanup deletes in recipe order, builds all nested archives through the same exact-verification engine, and emits a validated mod.toml package plus a retained operation audit. XML and delete operations in a new archive are accepted only after the target file is available at that point in the recipe. XML output is reparsed and canonically verified before archive construction. Official XML add/replace/remove commands can also be compiled against one explicitly selected existing outer RPF. Every XPath must select exactly one textual .xml/.meta element; the compiler blocks DTDs/entities and unbounded selectors, preserves operation order and encoding, reparses and canonically verifies each result, coalesces repeated target edits, and emits a retained payload bundle plus the normal inert multi-entry plan. Text/PSO edits, missing nested creation declarations, and creation roots deeper than one existing archive remain blocked.
...
Every non-directory entry exposes Raw hex preview and Export binary patch workspace. The workspace keeps immutable original.bin, same-size editable.bin, exact archive/entry binding, and a hash-chained patch history. SDK Console commands inspect bounded offsets, apply expected-byte patches, retain undo operations, and build a changed-range report. Plan replacement from binary workspace refuses source/archive drift, a broken history chain, size changes, more than 4 MiB of changed bytes, or a workspace exported from any other entry; applying remains a separate reviewed transaction.

GXT2 entries expose Export GXT2 text workspace. The searchable desktop table supports UTF-8 text edits, unique 32-bit hash additions, removal, local undo, and verified builds; File > Open GXT2 text workspace resumes an existing project. The same operations are available to the SDK Console and agent API. Both little-endian 2TXG markers, offsets, terminators, source snapshot, contiguous history hashes, outer archive hash, virtual entry, and edition are revalidated before Plan replacement from GXT2 workspace can publish an inert plan.
