...
File > RPF package node graph opens the visual authoring workflow. Import a loose source tree or start from an empty archive card. Purple archive, green directory, and blue source-file cards expose containment ports; drag cards to position them and drag a parent output port onto a child card to validate and save a reparent operation. The inspector adds, renames, removes, searches, lays out, validates, materializes, and exactly builds the graph. One versioned JSON document backs the canvas and the complete create/inspect/validate/add/rename/reparent/position/layout/remove/refresh/materialize/build CLI and agent-API surface. Graph removal never deletes source files, and changed sources require an explicit hash refresh before validation or build.
