...
OIV preview recognizes the official XML add/replace/remove grammar as a compile workflow while keeping text, PSO, unbounded archive-creation, ambiguous XPath, and unknown operations blocked. Select the matching outer RPF to compile supported XML recipes into reparsed, canonically verified payloads and a hash-bound inert plan; the source archive is not written. Bounded createIfNotExist recipes can become exactly verified managed packages, while exact adds/replacements/deletes inside existing nested RPF trees can be exported as payload-backed batch manifests.""",
    ...
    HelpTopic(
        ...
        """Open a package folder or supported archive, search its inventory, and select an asset. Images and text preview directly. Supported Rockstar resources receive header analysis, structured CodeWalker XML, and texture contact sheets when possible. YDR, YDD, and YFT model resources also receive a bounded indexed-geometry diagnostic view with drawable, LOD, bounds, vertex, and triangle statistics. YBN collision resources visualize world-positioned triangle meshes and diagnostic primitive hulls, with geometry, material, polygon-type, vertex, and bounds counts. These are inspection views rather than shader- or physics-engine-accurate game renderers; unsupported/custom layouts are skipped instead of guessed.

SDK Console command inspect-native-asset publishes the same analysis through JSON and can create a new portable folder containing structured text, a PNG preview, and report.json. The command is also discoverable through the local structured Agent API.
...
OIV preview recognizes bounded createIfNotExist recipes instead of executing them. The CLI's --created-rpf-package workflow extracts only declared content members into retained loose authoring sources, builds all nested archives through the same exact-verification engine, and emits a validated mod.toml package. Official XML add/replace/remove commands can be compiled against one explicitly selected existing outer RPF. Every XPath must select exactly one textual .xml/.meta element; the compiler blocks DTDs/entities and unbounded selectors, preserves operation order and encoding, reparses and canonically verifies each result, coalesces repeated target edits, and emits a retained payload bundle plus the normal inert multi-entry plan. Text/PSO edits, XML inside newly created archives, deletes inside a new archive, missing nested creation declarations, and creation roots deeper than one existing archive remain blocked.
