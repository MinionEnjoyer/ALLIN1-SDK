...
def test_launcher_bridge_never_falls_back_to_sibling_tk_checkout(
    ...
    with pytest.raises(ValueError, match="SDK works without the Launcher"):
        launcher_bridge.launcher_process_command(
            sdk, "studio.pagani", traffic_requested=False, environment={},
        )


def test_launcher_bridge_discovers_only_the_react_shell(tmp_path, monkeypatch):
    shell = tmp_path / "allin1-launcher-desktop.exe"
    shell.write_bytes(b"not executed")
    searched = []
    def which(name):
        searched.append(name)
        return str(shell)
    monkeypatch.setattr(launcher_bridge.shutil, "which", which)
    command, cwd = launcher_bridge.launcher_process_command(tmp_path, "studio.pagani",
        traffic_requested=True, environment={})
...
def test_launcher_bridge_fails_closed_and_popen_receives_argv(
    ...
    assert calls[0][1] == {"cwd": None, "close_fds": True,
                          "creationflags": getattr(launcher_bridge.subprocess, "CREATE_NO_WINDOW", 0)}
